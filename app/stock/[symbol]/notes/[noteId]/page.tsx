...
import { getFmpQuote, getNoteByIdAction, saveNoteAction, deleteNoteAction, getStockForUserAction, getFmpHistoricalData } from "@/app/actions" // Import all necessary Server Actions
...
export default function NoteEditPage({ params }: { params: { symbol: string; noteId: string } }) {
  ...
  useEffect(() => {
    ...
    async function fetchHistory() {
      ...
      const to = toYMD(today.toISOString());
      const fromDate = new Date(today);
      fromDate.setFullYear(today.getFullYear() - 1);
      const from = toYMD(fromDate.toISOString());
      const prices = await getFmpHistoricalData(symbol, from, to);
      setHistoricalPrices(prices.map(p => ({ date: toYMD(p.date), price: p.price })));
    }
    fetchHistory();
  }, [symbol]);

  // 计算笔记价格
  function getNotePrice(noteDate: string) {
    const ymd = toYMD(noteDate);
    const priceMap = Object.fromEntries(historicalPrices.map(p => [p.date, p.price]));
    if (priceMap[ymd] !== undefined) return priceMap[ymd];
    // 没有则找往前最近有价格的那天
    const allDates = Object.keys(priceMap).sort();
    const noteTimestamp = new Date(ymd).getTime();
    ...
    return closestPrecedingDate ? priceMap[closestPrecedingDate] : null;
  }
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <div className="text-muted-foreground text-sm mt-2">
            <p>
              股票: {stockName} ({symbol}) {getNotePrice(createdAt) !== null ? `$${getNotePrice(createdAt).toFixed(2)}` : ""}
            </p>
