...
export async function getUserStocks(userId: string): Promise<UserStock[]> {
  try {
    ...
    if (e instanceof TypeError && e.message === "Failed to fetch") {
      console.error(
        "Network error fetching user stocks from Supabase. Please check your internet connection and ensure NEXT_PUBLIC_SUPABASE_URL is correctly configured and accessible:",
        e,
      )
    } else if (e.message && e.message.includes("Too Many R")) {
